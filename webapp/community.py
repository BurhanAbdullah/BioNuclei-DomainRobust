...
def _run(job_id: str, image_path: Path, original_name: str, research_consent: bool, algorithm_profile: str, *, channel: int, time: int, z: int, field: int) -> None:
    ...
    try:
        ...
        result.update({"analysis_profile": algorithm_profile, "scientific_execution": True, "input_sha256": input_sha256, "source_filename": original_name, "input_metadata": input_metadata})
        ...
        metadata = {"original_filename": original_name, "algorithm_profile": algorithm_profile, "checkpoint_configured": True, "input_metadata": input_metadata}
        if research_consent:
...
def create_job(data: bytes, original_name: str, research_consent: bool, algorithm_profile: str, *, channel: int = 0, time: int = 0, z: int = 0, field: int = 0) -> str:
    ...
    with DB_LOCK, _db() as conn:
        conn.execute("INSERT INTO jobs(id,status,created_at,updated_at,expires_at,input_name,retained_copy,research_consent,algorithm_profile) VALUES(?,?,?,?,?,?,?,?,?)", (job_id, "queued", _now().isoformat(), _now().isoformat(), expires.isoformat(), original_name, 0, int(research_consent), algorithm_profile))
        conn.commit()
...
def serialize(row: sqlite3.Row) -> dict[str, object]:
    ...
    return {"job_id": row["id"], "status": row["status"], "created_at": row["created_at"], "updated_at": row["updated_at"], "expires_at": row["expires_at"], "input_name": row["input_name"], "input_sha256": row["input_sha256"], "research_consent": bool(row["research_consent"]), "retained_copy": bool(row["retained_copy"]), "algorithm_profile": row["algorithm_profile"], "result": result, "error": row["error"], "downloads": {"zip": f"/jobs/{row['id']}/download", "results": f"/jobs/{row['id']}/files/results.json"} if row["status"] == "completed" else {}}
