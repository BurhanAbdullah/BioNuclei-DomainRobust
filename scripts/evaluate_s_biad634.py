...
def to_model_grayscale(image):
    """Convert S-BIAD634 grayscale or RGB TIFFs to the single channel expected by the baseline."""
    x=np.asarray(image)
...
def main():
    ...
    ms=('dice','iou','aji','boundary_f1'); summary={'experiment':'zero_shot_bbbc039_to_s_biad634','n_images':len(results),'mean':{m:float(np.mean([r[m] for r in results])) for m in ms},'median':{m:float(np.median([r[m] for r in results])) for m in ms},'per_image':results,'checkpoint_seed':checkpoint.get('seed'),'tile_size':a.tile_size,'overlap':a.overlap}; a.output.mkdir(parents=True,exist_ok=True); (a.output/'metrics.json').write_text(json.dumps(summary,indent=2)+'\n'); print(json.dumps(summary['mean'],indent=2))
