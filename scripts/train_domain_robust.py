#!/usr/bin/env python3
"""Train a source-only domain-randomized Boundary U-Net robustness model."""
...
def main():
    ...
    for ep in range(1,int(a.epochs or c['training']['epochs'])+1):
        ...
        torch.save({'model':model.state_dict(),'config':c,'seed':int(c['seed']),'epochs_run':ep,'method':'source_only_intensity_domain_randomization'},a.output/'last.pt')
    (a.output/'train_history.json').write_text(json.dumps(hist,indent=2)+'\n')
    (a.output/'method_record.json').write_text(json.dumps({'method':'source_only_intensity_domain_randomization','target_data_used_for_training':False,'seed':int(c['seed'])},indent=2)+'\n')
