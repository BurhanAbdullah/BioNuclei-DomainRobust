...
def _pdf_font() -> tuple[str, str]:
    """Prefer configured Times New Roman files and use a Times fallback."""
    try:
...
def _expert_interpretation(payload: dict[str, Any]) -> list[str]:
    ...
    if count is not None:
        paragraphs.append(
            f"The segmentation stage identified {_clean(count)} nuclear objects in the analysed field. This count describes objects detected by the computational pipeline and should not be interpreted as a biological ground truth without experimental validation."
        )
    if morphology:
        paragraphs.append(
            "Morphological measurements provide a population level description of nuclear geometry. Mean area, perimeter, eccentricity, solidity and circularity should be interpreted together because each quantity captures a different aspect of nuclear shape. Increased dispersion can indicate biological heterogeneity, acquisition variation, segmentation uncertainty, or a combination of these factors."
        )
    if intensity:
        paragraphs.append(
            "Nuclear intensity measurements describe the measured fluorescence signal within segmented nuclear regions. Intensity differences can reflect biological signal variation, staining efficiency, exposure, background, photobleaching, detector response, or segmentation boundaries. Quantitative comparisons between samples therefore require consistent acquisition and normalization procedures."
        )
    paragraphs.append(
        f"The image profile reports a median intensity of {_fmt(profile.get('median'))}, a standard deviation of {_fmt(profile.get('std'))}, and a saturation fraction of {_fmt(profile.get('saturation_fraction'))}. The CNN reported {_clean(cnn.get('status', 'NOT RUN'))} for the analysisability assessment. These measurements provide context for interpreting the downstream results."
    )
    paragraphs.append(
        "The final interpretation is intentionally conservative. The computational evidence supports quantitative characterization of detected nuclear objects, but morphology and intensity measurements alone do not establish a specific biological mechanism, disease state, treatment response, or clinical diagnosis."
    )
    return paragraphs
...
def _build_pdf(payload: dict[str, Any], output_dir: Path) -> Path:
    ...
    doc = SimpleDocTemplate(
        ...
        title="BioNuclei Quantitative Bioimaging Analysis Report",
        author="BioNuclei",
    )
    ...
    story = [
        Paragraph("BioNuclei Quantitative Bioimaging Analysis Report", title),
        Paragraph("Evidence based nuclear image analysis and scientific interpretation", subtitle),
    ]

    summary_data = [
        ["Analysis status", status],
        ["Nuclei detected", _clean(count)],
        ["Image dimensions", _clean(profile.get("shape", "Not available"))],
        ["Image data type", _clean(profile.get("dtype", "Not available"))],
        ["Analysis modules", modules],
        ["CNN analysisability", _clean(cnn.get("status", "NOT RUN"))],
    ]
    table = Table(summary_data, colWidths=[62 * mm, 103 * mm], repeatRows=0)
    ...
    for text in _expert_interpretation(payload)[:2]:
        story.append(Paragraph(html.escape(_clean(text)), body))

    story.append(Paragraph("2. Sample and Image Information", heading))
    image_rows = [
        ["Source filename", _clean(payload.get("source_filename", "Not available"))],
        ["Image shape", _clean(profile.get("shape", "Not available"))],
        ["Data type", _clean(profile.get("dtype", "Not available"))],
        ["Minimum intensity", _fmt(profile.get("min"))],
        ["Median intensity", _fmt(profile.get("median"))],
        ["Maximum intensity", _fmt(profile.get("max"))],
        ["Mean intensity", _fmt(profile.get("mean"))],
        ["Intensity standard deviation", _fmt(profile.get("std"))],
        ["Zero fraction", _fmt(profile.get("zero_fraction"))],
        ["Saturation fraction", _fmt(profile.get("saturation_fraction"))],
    ]
    info = Table(image_rows, colWidths=[62 * mm, 103 * mm])
    info.setStyle(TableStyle([
        ("FONTNAME", (0, 0), (-1, -1), regular), ("FONTNAME", (0, 0), (0, -1), bold),
        ("FONTSIZE", (0, 0), (-1, -1), 9), ("GRID", (0, 0), (-1, -1), 0.35, colors.grey),
    ...
    quality_text = (
        f"The input quality assessment found finite values equal to {_clean(profile.get('finite', 'Not available'))}, "
        f"zero fraction {_fmt(profile.get('zero_fraction'))}, and saturation fraction {_fmt(profile.get('saturation_fraction'))}. "
        "These measurements are reported as acquisition context and are not themselves evidence of biological abnormality."
    )
    ...
    cnn_text = (
        f"The released {_clean(cnn.get('architecture', model.get('architecture', 'Boundary U Net')))} was evaluated before full segmentation. "
        f"The gate status was {_clean(cnn.get('status', 'NOT RUN'))}. Foreground fraction was {_fmt(cnn.get('foreground_fraction'))}, "
        f"mean class confidence was {_fmt(cnn.get('mean_class_confidence'))}, mean entropy was {_fmt(cnn.get('mean_entropy'))}, "
        f"and connected foreground components were {_clean(cnn.get('connected_components', 'Not available'))}. "
        "This gate is an analysisability check and is not a calibrated probability of correctness."
    )
    story.append(Paragraph(html.escape(_clean(cnn_text)), body))
    if cnn.get("reasons"):
        story.append(Paragraph("CNN quality notes: " + html.escape(_clean(" ".join(cnn["reasons"]))), body))

    story.append(Paragraph("5. Nuclear Segmentation", heading))
    story.append(Paragraph(
        html.escape(_clean(
            f"The scientific runner used the {_clean(model.get('architecture', 'Boundary U Net'))} model in evaluation mode. "
            f"The measured nuclear object count was {_clean(count)}. The segmentation result should be inspected visually, particularly for touching objects, fragmented objects, weak boundaries, and objects near the image border."
        )), body
    )

    story.append(Paragraph("6. Nuclear Morphology", heading))
    morph_rows = [
        ["Mean area", _fmt(morphology.get("mean_area"))],
        ["Median area", _fmt(morphology.get("median_area"))],
        ["Mean perimeter", _fmt(morphology.get("mean_perimeter"))],
        ["Mean eccentricity", _fmt(morphology.get("mean_eccentricity"))],
        ["Mean solidity", _fmt(morphology.get("mean_solidity"))],
        ["Mean circularity", _fmt(morphology.get("mean_circularity"))],
    ]
    mt = Table(morph_rows, colWidths=[62 * mm, 103 * mm])
    mt.setStyle(TableStyle([("FONTNAME", (0, 0), (-1, -1), regular), ("FONTNAME", (0, 0), (0, -1), bold),
                            ("FONTSIZE", (0, 0), (-1, -1), 9), ("GRID", (0, 0), (-1, -1), 0.35, colors.grey),
                            ("LEFTPADDING", (0, 0), (-1, -1), 6), ("RIGHTPADDING", (0, 0), (-1, -1), 6),
                            ("TOPPADDING", (0, 0), (-1, -1), 4), ("BOTTOMPADDING", (0, 0), (-1, -1), 4)]))
    story.append(mt)
    for text in _expert_interpretation(payload)[2:3]:
        story.append(Paragraph(html.escape(_clean(text)), body))

    story.append(Paragraph("7. Nuclear Intensity", heading))
    int_rows = [
        ["Mean nuclear intensity", _fmt(intensity.get("mean_nuclear_intensity"))],
        ["Median nuclear intensity", _fmt(intensity.get("median_nuclear_intensity"))],
        ["Mean maximum intensity", _fmt(intensity.get("mean_max_intensity"))],
    ]
    it = Table(int_rows, colWidths=[62 * mm, 103 * mm])
    it.setStyle(TableStyle([("FONTNAME", (0, 0), (-1, -1), regular), ("FONTNAME", (0, 0), (0, -1), bold),
                            ("FONTSIZE", (0, 0), (-1, -1), 9), ("GRID", (0, 0), (-1, -1), 0.35, colors.grey),
                            ("LEFTPADDING", (0, 0), (-1, -1), 6), ("RIGHTPADDING", (0, 0), (-1, -1), 6),
                            ("TOPPADDING", (0, 0), (-1, -1), 4), ("BOTTOMPADDING", (0, 0), (-1, -1), 4)]))
    story.append(it)
    for text in _expert_interpretation(payload)[3:4]:
        story.append(Paragraph(html.escape(_clean(text)), body))

    story.append(PageBreak())
    story.append(Paragraph("8. Integrated Bioimaging Interpretation", heading))
    for text in _expert_interpretation(payload):
        story.append(Paragraph(html.escape(_clean(text)), body))

    story.append(Paragraph("9. Quality Control and Limitations", heading))
    limitations = [
        "The CNN analysisability gate is not a calibrated diagnostic classifier.",
        "Object counts and morphology depend on segmentation quality and image acquisition conditions.",
        "Fluorescence intensity is affected by acquisition and staining conditions and should be compared only under controlled protocols.",
        "Quantitative morphology does not establish a specific biological mechanism without appropriate experimental controls.",
        "This software does not provide clinical diagnosis.",
    ]
    for item in limitations:
        story.append(Paragraph(html.escape(_clean(item)), body))

    story.append(Paragraph("10. AI Agent Evidence Chain", heading))
    roles = plan.get("agent_roles", [])
    if roles:
        for role in roles:
            story.append(Paragraph(html.escape(_clean(role)), body))
    else:
        story.append(Paragraph("The executed adaptive plan did not provide agent role metadata.", body))

    story.append(Paragraph("11. Reproducibility and Provenance", heading))
    provenance = [
        ["Model architecture", _clean(model.get("architecture", "Boundary U Net"))],
        ["Training reference", _clean(model.get("training_reference", "BBBC039v1"))],
        ["Input SHA256", _clean(payload.get("input_sha256", "Not available"))],
        ["Analysis profile", _clean(payload.get("analysis_profile", "Not available"))],
    ]
    pt = Table(provenance, colWidths=[62 * mm, 103 * mm])
    pt.setStyle(TableStyle([("FONTNAME", (0, 0), (-1, -1), regular), ("FONTNAME", (0, 0), (0, -1), bold),
                            ("FONTSIZE", (0, 0), (-1, -1), 8.5), ("GRID", (0, 0), (-1, -1), 0.35, colors.grey),
                            ("LEFTPADDING", (0, 0), (-1, -1), 6), ("RIGHTPADDING", (0, 0), (-1, -1), 6),
                            ("TOPPADDING", (0, 0), (-1, -1), 4), ("BOTTOMPADDING", (0, 0), (-1, -1), 4)]))
    story.append(pt)
    ...
    story.append(Paragraph(
        html.escape(_clean(
            "The report provides a quantitative description of the analysed microscopy field using the BioNuclei computational pipeline. The conclusions are limited to the measured image evidence and should be interpreted together with acquisition metadata, experimental controls, and visual inspection of the segmentation output."
        )), body
    )
...
body{{font-family:'Times New Roman',Times,serif;max-width:850px;margin:40px auto;padding:0 28px;color:#111;background:#fff;line-height:1.55}}
h1,h2,h3{{font-family:'Times New Roman',Times,serif;color:#111}}h1{{font-size:28px}}h2{{font-size:18px;margin-top:28px}}table{{border-collapse:collapse;width:100%;margin:12px 0 20px}}td{{border:1px solid #777;padding:8px;vertical-align:top}}td:first-child{{font-weight:bold;width:38%}}.small{{font-size:11px}}.note{{border:1px solid #777;padding:12px;background:#fff;color:#111}}
</style></head><body><h1>BioNuclei Quantitative Bioimaging Analysis Report</h1><p><b>Analysis status:</b> {esc(status)}</p>
...
<h2>8. AI Agent Evidence Chain</h2><p>{esc(', '.join(_clean(x) for x in plan.get('agent_roles', [])))}</p>
