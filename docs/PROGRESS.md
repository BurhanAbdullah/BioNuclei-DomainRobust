...
Fresh corrected E5 run `33943463177` is currently in progress. All three preregistered variants (`full_frozen_e4`, `no_intensity_randomization`, `no_contrast`) have completed source/split verification and target inventory; their actual variant execution remains in progress and the run currently has no uploaded artifacts. Therefore E5 is not accepted and no E5 metrics are promoted. The previous run `33927261340` remains an infrastructure-gate failure caused by artifact extraction layout and is not reused as a scientific E5 pass. The extraction-root fix is committed in `b31972cff9e4e32be9df201d70923177226546e2`.
